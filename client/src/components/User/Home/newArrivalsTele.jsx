...
const NewArrivalsTele = () => {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');
  
  const navigate = useNavigate();
  const categoryId = '691a40dd234098bbd959de39';
  ...
  return (
    <section className="w-full py-10 px-5 bg-gray-50">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold text-[#972323]">NEW ARRAIVALS</h2>
            ...
            {displayedProducts.map((product) => {
              ...
              return (
                <div key={product._id} className="bg-white p-4 rounded-lg shadow-md flex flex-col items-center relative">
                  ...
                  {/* FIXED: Pass the specific product to handleAddToCart */}
                  <button
                    onClick={() => handleAddToCart(product)} //Pass the product
...
export default NewArrivalsTele;
